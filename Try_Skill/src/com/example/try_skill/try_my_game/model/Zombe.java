...
//		battleableSpriteType = BattleableSpriteType.Enemy;
		...
		MovementActionInfo info = new MovementActionInfo(3000, BattleUtil.changeToNew(1, getAttributeInfo().getBattleInviable()), -2, 0, "", null, false);
...
//		if(battleSpriteInjureType == BattleSpriteInjureType.Frozen){
		...
		if(getAttributeInfo().checkHasEffectOrNotByEffectType(InjureType.Fire)){
...
//				paint.setAlpha(alpha);
			...
			if(paint==null)
				paint = new Paint();
			paint.setColorFilter(ColorFilterBuilder.getEffectColor(InjureType.Fire));
			
			Log.e("zoobe", "draw fire");
		}
		
		if(getAttributeInfo().checkHasEffectOrNotByEffectType(InjureType.Frozen)){
			if(paint==null)
				paint = new Paint();
...
//			paint.setColorFilter(colorMatrixColorFilter);
			...
			paint.setColorFilter(ColorFilterBuilder.getEffectColor(InjureType.Frozen));
...
//			paint.setColorFilter(c);
		...
		if(getAttributeInfo().checkHasEffectOrNotByEffectType(InjureType.Normal)){
			if(paint==null)
				paint = new Paint();
...
//			isInjure = false;
	...
	public void updateInfoFromAtrributeInfoDetail(){
		super.updateInfoFromAtrributeInfoDetail();
//		getAttributeInfo().getBattleInviable();
		setNew();
	}
...
//	}
	...
	public void setNew(){
		actions.get(SheepMove.Shoot.getName()).frameTime = BattleUtil.changeToNew(actions.get(SheepMove.Shoot.getName()).frameTime, getAttributeInfo().getBattleInviable());
		actions.get(SheepMove.Shoot.getName()).initUpdateTime();
		for(MovementActionInfo info : getMovementAction().getMovementInfoList()){
			int newDelay = BattleUtil.changeToNew((int)info.getDelay(), getAttributeInfo().getBattleInviable());
			Log.e("newDelay", "newDelay:"+newDelay);
			info.setDelay(newDelay);
		}
//		battleInviable = 1.0f;
		getAttributeInfo().setBattleInviable(1.0f);
	}
